...
app.post("/api/v1/oauth/token", (req, res) =>
  apiManager.tokenController(req, res),
);

app.post("/api/v1/login", (req, res) => apiManager.loginUser(req, res));

app.post("/api/v1/auth-payment", (req, res) =>
  apiManager.authorizePayment(req, res),
);
...
app.get("/api/v1/user/profile", (req, res) =>
  apiManager.getUserProfile(req, res),
);

app.post("/api/v1/bank/topup", (req, res) => apiManager.topUpBalance(req, res));
app.get("/api/v1/bank/config", (req, res) => apiManager.getConfig(req, res));

app.use(express.static("public"));

const PORT = process.env["PORT"] || 3000;
app.listen(PORT, () => {
  console.log(`Pixel Bank Server running at: http://localhost:${PORT}`);
});
